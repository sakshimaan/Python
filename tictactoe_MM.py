...
def maximize(): #(-1 loss,1 win,0 tie)
    max_value = -2  
    pos_max_x = None
    pos_max_y = None
...
def minimize(): #(1 loss,-1 win,0 tie)
    min_value = 2
    pos_min_x = None
    pos_min_y = None
    result = finished()
    ...
    def send_message(self):
        try:
            ...
            while (True):
                ...
                in_range = True
                new_pos = True
                pos = input('Enter the position you want to make a move in :')
                if not pos.isdigit():
                    digit =False
